import React from 'react';
import variant1 from '../assets/images/color.svg';
import variant2 from '../assets/images/wheel.svg';
import variant3 from '../assets/images/Implements.svg';
import tractor from '../assets/images/home/tractor.jpeg';
import tractor1 from '../assets/images/home/tractor1.jpg';
import cat1 from '../assets/images/Image_27.png';
import cat2 from '../assets/images/Mask_Group_5.png';
import cat3 from '../assets/images/Mask_Group_7.png';
// Import css files
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";
import Slider from "react-slick";

const Implementsvariant = () => {
    var settings = {
        dots: true,
        infinite: true,
        speed: 500,
        slidesToShow: 1,
        slidesToScroll: 1,
        arrows:false
    };

    return (
        <div>
            <div className="carousel pdp">
                <Slider {...settings}>
                    <div className="p-pdp-slidercard">
                        <div className="card">
                            <img src={tractor} />
                        </div>
                    </div>
                    <div className="p-pdp-slidercard">
                        <div className="card">
                            <img src={tractor1} />
                        </div>
                    </div>
                    <div className="p-pdp-slidercard">
                        <div className="card">
                            <img src={cat1} />
                        </div>
                    </div>
                    <div className="p-pdp-slidercard">
                        <div className="card">
                            <img src={cat2} />
                        </div>
                    </div>

                </Slider>
            </div>
            <div className="p-pdp">

                <div className="p-pdp-details card">
                    <div className="name">5036D 36HP, 2100 RPM Tractor</div>
                    <div className="rating">
                        <span className="rated">&#9733;</span>
                        <span className="rated">&#9733;</span>
                        <span className="rated">&#9733;</span>
                        <span className="rated">&#9733;</span>
                        <span>&#9733;</span>
                    </div>
                    <div className="price hide">Rs.4.5 Lakhs</div>
                </div>

                <div className="variants-wrapper">
                    <div className="title">Implments</div>
                    <ul className="variants-wrapper--list">
                        <li>
                            <label className="gfg-input input-checkbox">
                                <input type="checkbox" name="color-1" checked/>
                                <span className="checkmark"></span>

                                <div className="img">
                                    <img src={cat1} />
                                </div>
                                <div className="variant-name">
                                    Agriculture Tractor Cultivator
                                </div>
                            </label>
                        </li>

                        <li>
                            <label className="gfg-input input-checkbox">
                                <input type="checkbox" name="color-1" checked/>
                                <span className="checkmark"></span>

                                <div className="img">
                                    <img src={cat2} />
                                </div>
                                <div className="variant-name">
                                    Tractor Implements Rotavator, Cultivator
                                </div>
                            </label>
                        </li>

                        <li>
                            <label className="gfg-input input-checkbox">
                                <input type="checkbox" name="color-1" checked/>
                                <span className="checkmark"></span>

                                <div className="img">
                                    <img src={cat3} />
                                </div>
                                <div className="variant-name">
                                    CP69Q Cotton Harvesters
                                </div>
                            </label>
                        </li>
                    </ul>
                        <a href="/" className="more-implements icon">More Implements</a>
                </div>

                

            </div>
        </div>
    );
}
export default Implementsvariant;